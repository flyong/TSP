...
def plotNetwork(figureName,relation_sheet):
    testIter = 1
    correlation_path = r'/home/vsc/workspace/IDT3/Cor/dataprocess/data/correlation.xlsx'
    
    position_path = r'/home/vsc/workspace/IDT3/Cor/dataprocess/data/position.xlsx'
    position_sheet = ['GS_XY','TS_A1_M_XY']

    # Read correlation matrix
    correlation_matrix = read_excel(correlation_path, relation_sheet)
    groundPosition = read_excel(position_path, position_sheet[0])
    tunnelPosition = read_excel(position_path, position_sheet[1])
    ...
    for tunnelID in tunnelSensorID:
        j=0
        for groundID in groundSensorID:
            relationMatrix[i][j] = correlation_matrix.loc[(correlation_matrix['tunnelID']==tunnelID), groundID].values[0]
            j+=1
        i+=1
    ...
    # Show the plot
    figname='Cor/dataprocess/image/graph'+str(figureName)+'.png'
    plt.savefig(figname)
    print('Plot saved')
